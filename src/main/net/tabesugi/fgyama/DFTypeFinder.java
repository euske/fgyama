...
public class DFTypeFinder {
    ...
    private class StringIndex {
        ...
        public String getString(char e) {
            int i = this.s.indexOf(e, this.i);
            String s = this.s.substring(this.i, i);
            this.i = i+1;
            return s;
        }
    }

    private DFType resolveJNI(StringIndex x)
        throws TypeNotFound {
        char c = x.getChar();
        switch (c) {
        case 'B': case 'C': case 'S': case 'I': case 'J':
        case 'F': case 'D': case 'Z': case 'V':
            return DFBasicType.getType(c);
        case 'L':
            String name = x.getString(';');
            if (name.endsWith(">")) {
                int i = name.lastIndexOf('<');
                List<DFType> types = new ArrayList<DFType>();
                StringIndex y = new StringIndex(name.substring(i+1, name.length()));
                while (true) {
                    DFType argType = this.resolveJNI(y);
                ...
                DFKlass genericKlass = this.lookupKlass(name.replace('/','.'));
                return genericKlass.getParamKlass(argTypes);
            } else {
                return this.lookupKlass(name.replace('/','.'));
            }
        case '[':
            int ndims = 1;
            while (x.peekChar() == '[') {
            ...
            DFType elemType = this.resolveJNI(x);
            return new DFArrayType(elemType, ndims);
        case '+':
        case '-':
            return this.resolveJNI(x);
        case '*':
            return DFRootTypeSpace.OBJECT_KLASS;
        case 'T':
            return this.lookupParamType(x.getString(';'));
        case ')':
        case '>':
            return null;
        case '(':
            List<DFType> types = new ArrayList<DFType>();
            while (true) {
                DFType argType = this.resolveJNI(x);
                if (argType == null) break;
                types.add(argType);
            }
            DFType[] argTypes = new DFType[types.size()];
            types.toArray(argTypes);
            DFType returnType = this.resolveJNI(x);
            return new DFMethodType(argTypes, returnType);
        default:
            throw new TypeNotFound(x.s);
        }
    }

    public DFType resolveJNI(String s)
        throws TypeNotFound {
        return resolveJNI(new StringIndex(s));
    }
